...
clear
echo -e "${GREEN}==========================================${RESET}"
echo -e "${GREEN}Realm 面板 (IPv6 智能检测版) 一键部署     ${RESET}"
echo -e "${GREEN}==========================================${RESET}"
...
async fn index_page(cookies: Cookies, State(state): State<Arc<AppState>>) -> Response {
    ...
    // 获取环境变量中的 IPv6 状态并注入前端
    let ipv6_enabled = std::env::var("ENABLE_IPV6").unwrap_or("false".to_string());

    let html = DASHBOARD_HTML
        .replace("{{USER}}", &data.admin.username)
        .replace("{{BG_PC}}", &data.admin.bg_pc)
        .replace("{{BG_MOBILE}}", &data.admin.bg_mobile)
        .replace("{{IPV6_SUPPORT}}", &ipv6_enabled);
...
# --- 新增：IPv6 检测逻辑 ---
echo -e -n "${CYAN}>>> 检测 IPv6 支持情况...${RESET}"
# 方法：检测是否有 Global Scope 的 IPv6 地址
if ip -6 addr show scope global | grep -q "inet6"; then
    HAS_IPV6="true"
    echo -e "${GREEN} [支持]${RESET}"
else
    HAS_IPV6="false"
    echo -e "${YELLOW} [不支持 (面板将禁用IPv6输入)]${RESET}"
fi
...
echo -e "------------------------------------------"
echo -e "当前服务器 IPv6 状态: ${YELLOW}$HAS_IPV6${RESET}"
