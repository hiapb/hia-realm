...
clear
echo -e "${GREEN}==========================================${RESET}"
echo -e "${GREEN}   Realm 面板 (编译修复最终版) 一键部署   ${RESET}"
echo -e "${GREEN}==========================================${RESET}"
...
# 3. 生成代码
run_step "准备源代码" "rm -rf '$WORK_DIR' && mkdir -p '$WORK_DIR/src'"
cd "$WORK_DIR"

# ⚠️ 关键修改：移除了 network-interface 依赖，改为纯原生实现，保证编译成功
cat > Cargo.toml <<EOF
[package]
name = "realm-panel"
version = "3.5.0"
...
async fn add_rule(c: Cookies, State(s): State<Arc<AppState>>, Json(req): Json<AddRuleReq>) -> Response {
    ...
    if req.remote.contains('[') || req.remote.matches(':').count() > 1 {
        if !has_ipv6() { return (StatusCode::BAD_REQUEST, "本机无 IPv6 环境，无法添加 IPv6 目标！").into_response(); }
    }
