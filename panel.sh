...
clear
echo -e "${GREEN}==========================================${RESET}"
echo -e "${GREEN}   Realm 转发面板 (网络诊断版) 一键部署   ${RESET}"
echo -e "${GREEN}==========================================${RESET}"
...
use std::{fs, process::Command, sync::{Arc, Mutex}, path::Path as FilePath, time::{Duration, Instant}};
...
fn has_ipv6() -> bool {
    if let Ok(itfs) = NetworkInterface::show() {
        for itf in itfs {
            for addr in itf.addr {
                if addr.ip().is_ipv6() && !addr.ip().is_loopback() && !addr.ip().is_multicast() {
                    let ip_str = addr.ip().to_string();
                    if !ip_str.starts_with("fe80") { return true; }
                }
            }
        }
    }
...
async fn add_rule(c: Cookies, State(s): State<Arc<AppState>>, Json(req): Json<AddRuleReq>) -> Response {
    ...
    if req.remote.contains('[') || req.remote.matches(':').count() > 1 {
        if !has_ipv6() { return (StatusCode::BAD_REQUEST, "本机无外网IPv6环境，无法添加IPv6目标").into_response(); }
    }
...
async fn check_node(c: Cookies, State(s): State<Arc<AppState>>, Path(id): Path<String>) -> Response {
    ...
    if let Some(r) = d.rules.iter().find(|x| x.id == id) {
        ...
        match res {
            ...
            Ok(Err(e)) => Json(serde_json::json!({"ok":false, "msg": format!("连接拒绝/无法到达 ({})", e)})).into_response(),
