...
clear
echo -e "${GREEN}==========================================${RESET}"
echo -e "${GREEN}Realm 面板 (最终圆角修复版)   ${RESET}"
echo -e "${GREEN}==========================================${RESET}"
...
/* PC端 Header 样式：四角圆角 */
.pc-header{display:grid;grid-template-columns:1fr 1.5fr 1.5fr 2fr auto;gap:15px;padding:15px 20px;background:rgba(255,255,255,0.45);border-radius:18px;margin-bottom:12px;font-weight:bold;color:#6b7280;font-size:0.9rem;border:1px solid rgba(255,255,255,0.3);backdrop-filter:blur(10px)}
/* PC端 Rule Item 样式：四角圆角 */
.rule-item{background:rgba(255,255,255,0.45);border-radius:18px;margin-bottom:12px;padding:16px 20px;border:1px solid rgba(255,255,255,0.3);transition:0.3s;display:grid;grid-template-columns:1fr 1.5fr 1.5fr 2fr auto;align-items:center;gap:15px;box-shadow:0 2px 8px rgba(0,0,0,0.02)}
...
@media(max-width:768px){
    .pc-header{display:none} /* 移动端隐藏表头 */
    ...
    /* 移动端操作按钮：靠右、间距小 */
    .rule-item .actions{justify-content:flex-end;margin-top:10px;padding-top:15px;border-top:1px solid rgba(0,0,0,0.05);width:100%;gap:8px}
    .rule-item .actions .btn{flex:none;width:auto;padding:8px 14px}
}
...
<div class="container">
    <div class="card grid-add"><input id="n" placeholder="备注名称"><input id="l" placeholder="监听端口"><input id="r" placeholder="目标地址"><button class="btn btn-p" onclick="add()"><i class="fa fa-plus"></i> 添加规则</button></div>
    <div class="pc-header"><span>状态</span><span>备注</span><span>监听</span><span>目标</span><span>操作</span></div>
    <div class="rules-container" id="list"></div>
</div>
<div id="mSet" class="modal"><div class="modal-content"><h3><i class="fa fa-user-shield"></i> 面板设置</h3><label>管理账户</label><input id="su" placeholder="用户名" style="width:100%;margin-bottom:10px"><br><label>面板密码</label><input id="sp" type="password" placeholder="不改请留空" style="width:100%"><br><br><div class="actions" style="justify-content:space-between;display:flex"><button class="btn btn-g" style="flex:1" onclick="closeModal()">取消</button><button class="btn btn-p" style="flex:1" onclick="saveAcc()">保存更改</button></div></div></div>
<div id="mEdit" class="modal"><div class="modal-content"><h3><i class="fa fa-edit"></i> 修改规则</h3><label>备注</label><input id="en" style="width:100%;margin-bottom:10px"><label>监听</label><input id="el" style="width:100%;margin-bottom:10px"><label>目标</label><input id="er" style="width:100%"><br><br><div class="actions" style="justify-content:space-between;display:flex"><button class="btn btn-g" style="flex:1" onclick="closeModal()">取消</button><button class="btn btn-p" style="flex:1" onclick="saveEdit()">保存更新</button></div></div></div>
<script>
    ...
    rules.forEach(x=>{
        ...
        if(isMob){
            div.innerHTML=`
                ...
                <div class="mobile-row"><span class="mobile-label">目标</span>${x.remote}</div>
                <div class="actions">
                    <button class="btn btn-g" onclick="tog('${x.id}')"><i class="fa ${x.enabled?'fa-pause':'fa-play'}"></i></button>
                    <button class="btn btn-p" onclick="openEdit('${x.id}')"><i class="fa fa-pen"></i></button>
