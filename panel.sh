...
run_step "生成代码" "rm -rf '$WORK_DIR' && mkdir -p '$WORK_DIR/src'"
cd "$WORK_DIR"
...
fn load_or_init_data() -> AppData {
    ...
    if FilePath::new(REALM_CONFIG).exists() {
        if let Ok(c) = fs::read_to_string(REALM_CONFIG) {
            if let Ok(v) = c.parse::<toml::Value>() {
                 if let Some(eps) = v.get("endpoints").and_then(|v| v.as_array()) {
                     for ep in eps {
...
async fn login_action(c: Cookies, State(s): State<Arc<AppState>>, Form(f): Form<LoginParams>) -> Response {
    ...
    } else { Html("<script>alert('Error');window.location='/login'</script>").into_response() }
}
...
echo -e "地址 : ${YELLOW}http://${IP}:${PANEL_PORT}${RESET}"
echo -e "用户 : ${YELLOW}${DEFAULT_USER}${RESET}"
