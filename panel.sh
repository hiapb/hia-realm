...
clear
echo -e "${GREEN}==========================================${RESET}"
echo -e "${GREEN}   Realm 转发面板 (保活修复版) 一键部署   ${RESET}"
echo -e "${GREEN}==========================================${RESET}"
...
fn save_config_toml(data: &AppData) {
    ...
    // 如果没有启用的规则，添加一个本地的 Dummy 规则防止 Realm 退出
    ...
    let toml_str = toml::to_string(&config).unwrap(); // 肯定有值，unwrap 安全
...
echo -e ""
echo -e "${GREEN}==========================================${RESET}"
echo -e "${GREEN}✅ 部署完成！(已启用智能保活机制)${RESET}"
echo -e "${GREEN}==========================================${RESET}"
...
echo -e "------------------------------------------"
echo -e "机制说明："
echo -e "当所有规则被暂停时，系统会自动生成一个隐藏的"
echo -e "本地规则 (127.0.0.1:65534) 以保持 Realm 运行。"
echo -e "这样就彻底解决了暂停规则导致服务崩溃的问题。"
