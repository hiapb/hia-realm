...
clear
echo -e "${GREEN}==========================================${RESET}"
echo -e "${GREEN}   Realm 面板 (无限制稳定版) 一键部署     ${RESET}"
echo -e "${GREEN}==========================================${RESET}"
...
# 3. 生成代码
run_step "生成源代码" "rm -rf '$WORK_DIR' && mkdir -p '$WORK_DIR/src'"
cd "$WORK_DIR"
...
async fn main() {
    ...
    let app = Router::new()
        ...
        // 这里的 check_node 去掉了 Cookies 参数，避免编译报错
...
// 诊断接口：移除了 Cookies 参数，解决了编译报错
async fn check_node(State(s): State<Arc<AppState>>, Path(id): Path<String>) -> impl IntoResponse {
    let d = s.data.lock().unwrap(); 
