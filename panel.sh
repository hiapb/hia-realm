...
echo -e "${GREEN}>>> 正在部署 Realm Pro Plus (UI 重构版)...${RESET}"
...
/* Main Layout: Fixed header/add-rule, Scrolling table */
.container { 
...
.card-fixed { flex: 0 0 auto; } /* Input area */
.card-scroll { flex: 1 1 auto; overflow: hidden; display: flex; flex-direction: column; padding: 0; } /* Table area */
...
<div id="setModal" class="modal">
    <div class="modal-box">
        <div class="tab-header">
            <div class="tab-btn active" onclick="switchTab(0)">账号安全</div>
            <div class="tab-btn" onclick="switchTab(1)">界面背景</div>
        </div>
        
        <div class="tab-content active" id="tab0">
            <label>用户名</label><input id="set_u" value="{{USER}}">
            <label>新密码 (留空不改)</label><input id="set_p" type="password">
        ...
        <div class="tab-content" id="tab1">
            <label>PC端背景图 URL</label><input id="bg_pc" value="{{BG_PC}}">
            <label>移动端背景图 URL</label><input id="bg_mob" value="{{BG_MOBILE}}">
            <div style="margin-top:15px; text-align:right">
                <button class="btn btn-gray" onclick="closeModal()">取消</button>
                <button class="btn btn-primary" onclick="saveBg()">保存背景</button>
            </div>
        </div>
    </div>
</div>
...
<div id="editModal" class="modal">
    <div class="modal-box">
        ...
        <label>备注</label><input id="edit_n">
        <label>监听</label><input id="edit_l">
        <label>目标</label><input id="edit_r">
        <div style="margin-top:15px; text-align:right">
            <button class="btn btn-gray" onclick="closeModal()">取消</button>
...
function render(){
    ...
    rules.forEach(r=>{
        ...
        if(!r.enabled) row.className='row-paused';
        row.innerHTML = `
            <td data-label="状态"><span class="status-dot ${r.enabled?'bg-green':'bg-gray'}"></span>${r.enabled?'运行':'暂停'}</td>
            <td data-label="备注"><strong>${r.name}</strong></td>
            <td data-label="监听">${r.listen}</td>
            <td data-label="目标">${r.remote}</td>
            <td data-label="操作" style="text-align:right; gap:5px; display:flex; justify-content:flex-end;">
                <button class="btn btn-sm ${r.enabled?'btn-gray':'btn-primary'}" onclick="tog('${r.id}')"><i class="fas ${r.enabled?'fa-pause':'fa-play'}"></i></button>
                <button class="btn btn-sm btn-primary" onclick="openEdit('${r.id}')"><i class="fas fa-pen"></i></button>
...
async function saveEdit(){
    const body=JSON.stringify({name:$('edit_n').value,listen:$('edit_l').value,remote:$('edit_r').value});
    await fetch(`/api/rules/${$('edit_id').value}`,{method:'PUT',headers:{'Content-Type':'application/json'},body});
    $('editModal').style.display='none'; load();
}
...
async function saveAccount(){
    await fetch('/api/admin/account',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:$('set_u').value,password:$('set_p').value})});
    alert('账号更新，请重新登录'); location.reload();
}
async function saveBg(){
    await fetch('/api/admin/bg',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({bg_pc:$('bg_pc').value,bg_mobile:$('bg_mob').value})});
    alert('背景已更新'); location.reload();
}
...
# 3. 编译与安装
echo -e "${GREEN}>>> 开始编译美化版 (稍等片刻)...${RESET}"
cargo build --release
...
echo -e ""
echo -e "${GREEN}==========================================${RESET}"
echo -e "${GREEN}✨ Realm Pro Plus (UI重构版) 部署完成！${RESET}"
echo -e "${GREEN}==========================================${RESET}"
...
echo -e "------------------------------------------"
echo -e "功能升级："
echo -e "1. 登录页 100vh 锁定，杜绝滚动溢出。"
echo -e "2. 列表区域独立滚动，表头固定，支持海量规则。"
echo -e "3. 设置分离：账号与背景图独立保存。"
echo -e "4. 多端适配：PC/手机 自动切换背景图。"
