...
fn default_bg_mobile() -> String { "https://img.inim.im/file/1765444172962_1765443756179.jpg".to_string() }
...
<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no"><title>Realm Login</title><style>*{margin:0;padding:0;box-sizing:border-box}body{height:100vh;width:100vw;overflow:hidden;display:flex;justify-content:center;align-items:center;font-family:-apple-system,sans-serif;background:url('{{BG_PC}}') no-repeat center center/cover}@media(max-width:768px){body{background-image:url('{{BG_MOBILE}}')}}.overlay{position:absolute;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.3);backdrop-filter:blur(3px)}.box{position:relative;z-index:2;background:rgba(255,255,255,0.7);backdrop-filter:blur(10px);padding:2rem;border-radius:16px;box-shadow:0 8px 32px rgba(0,0,0,0.1);width:90%;max-width:350px;text-align:center;border:1px solid rgba(255,255,255,0.4)}h2{margin-bottom:1.5rem;color:#1a1a1a}input{width:100%;padding:12px;margin-bottom:1rem;border:1px solid rgba(0,0,0,0.1);border-radius:8px;outline:none;background:rgba(255,255,255,0.5)}button{width:100%;padding:12px;background:#2563eb;color:white;border:none;border-radius:8px;cursor:pointer;font-weight:bold;font-size:1rem;transition:0.2s}button:hover{background:#1d4ed8}</style></head><body><div class="overlay"></div><div class="box"><h2>Realm 转发面板</h2><form action="/login" method="post"><input type="text" name="username" placeholder="Username" required><input type="password" name="password" placeholder="Password" required><button type="submit">登录</button></form></div></body></html>
"#;

const DASHBOARD_HTML: &str = r#"
<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no, viewport-fit=cover"><title>Realm Panel</title><link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet"><style>:root{--primary:#2563eb;--danger:#ef4444;--success:#10b981}::-webkit-scrollbar{width:5px}::-webkit-scrollbar-thumb{background:rgba(0,0,0,0.2);border-radius:10px}*{box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;margin:0;padding:0;height:100vh;height:100dvh;overflow:hidden;background:url('{{BG_PC}}') no-repeat center center/cover;display:flex;flex-direction:column}@media(max-width:768px){body{background-image:url('{{BG_MOBILE}}')}}.overlay{position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.1);z-index:-1}.navbar{flex:0 0 auto;background:rgba(255,255,255,0.5);backdrop-filter:blur(15px);padding:0.8rem 1.5rem;display:flex;justify-content:space-between;align-items:center;z-index:10;border-bottom:1px solid rgba(255,255,255,0.2)}.brand{font-weight:800;font-size:1.1rem;color:#1a1a1a;display:flex;align-items:center;gap:10px}.container{flex:1 1 auto;display:flex;flex-direction:column;max-width:1100px;margin:0.8rem auto;width:100%;padding:0 0.8rem;overflow:hidden}.card{background:rgba(255,255,255,0.6);backdrop-filter:blur(12px);border-radius:12px;padding:1rem;box-shadow:0 8px 32px rgba(0,0,0,0.05);margin-bottom:0.8rem;border:1px solid rgba(255,255,255,0.3)}.card-scroll{flex:1 1 auto;overflow:hidden;display:flex;flex-direction:column;padding:0}.table-wrapper{flex:1;overflow-y:auto;padding:0 1rem}table{width:100%;border-collapse:collapse}thead th{position:sticky;top:0;background:rgba(255,255,255,0.1);backdrop-filter:blur(5px);z-index:5;padding:12px 5px;text-align:left;color:#4b5563;border-bottom:1px solid rgba(0,0,0,0.05)}td{padding:12px 5px;border-bottom:1px solid rgba(0,0,0,0.03);color:#1f2937;font-size:0.9rem}.btn{padding:8px 12px;border-radius:6px;border:none;cursor:pointer;color:white;display:inline-flex;align-items:center;gap:5px;font-size:0.85rem}.btn-primary{background:var(--primary)}.btn-danger{background:var(--danger)}.btn-gray{background:rgba(0,0,0,0.05);color:#1f2937}.grid-input{display:grid;grid-template-columns:1fr 1fr 1fr auto;gap:10px}input{padding:10px;border:1px solid rgba(0,0,0,0.1);border-radius:6px;outline:none;background:rgba(255,255,255,0.3)}input:focus{background:rgba(255,255,255,0.7);border-color:var(--primary)}.status-dot{height:8px;width:8px;border-radius:50%;display:inline-block;margin-right:6px}.bg-green{background:var(--success)}.bg-gray{background:#9ca3af}.empty-state{padding:50px;text-align:center;color:#6b7280}.modal{display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.4);z-index:100;justify-content:center;align-items:center;backdrop-filter:blur(4px)}.modal-box{background:rgba(255,255,255,0.8);backdrop-filter:blur(20px);width:90%;max-width:400px;padding:1.5rem;border-radius:15px;border:1px solid rgba(255,255,255,0.4)}@media(max-width:768px){.grid-input{grid-template-columns:1fr}.container{margin:0.5rem auto}.nav-text{display:none}thead{display:none}tr{display:flex;flex-direction:column;border:1px solid rgba(255,255,255,0.2);margin-bottom:10px;border-radius:10px;padding:10px;background:rgba(255,255,255,0.3)}td{border:none;padding:4px 0;display:flex;justify-content:space-between}td::before{content:attr(data-label);font-weight:bold;color:#6b7280;font-size:0.8rem}.table-wrapper{padding:0 5px}}</style></head><body><div class="overlay"></div><div class="navbar"><div class="brand"><i class="fas fa-bolt" style="color:var(--primary)"></i> <span class="nav-text">Realm 转发面板</span></div><div class="nav-actions"><button class="btn btn-gray" onclick="openSettings()"><i class="fas fa-cog"></i></button><form action="/logout" method="post" style="margin:0"><button class="btn btn-danger"><i class="fas fa-power-off"></i></button></form></div></div><div class="container"><div class="card"><div class="grid-input"><input id="n" placeholder="备注"><input id="l" placeholder="监听端口"><input id="r" placeholder="目标 (IP:端口)"><button class="btn btn-primary" onclick="add()"><i class="fas fa-plus"></i> 添加</button></div></div><div class="card card-scroll" style="padding:0"><div style="padding:10px 1rem;border-bottom:1px solid rgba(0,0,0,0.05);font-weight:bold">转发规则</div><div class="table-wrapper"><table id="ruleTable"><thead><tr><th>状态</th><th>备注</th><th>监听</th><th>目标</th><th style="text-align:right">操作</th></tr></thead><tbody id="list"></tbody></table><div id="emptyView" class="empty-state" style="display:none"><i class="fas fa-folder-open" style="display:block;font-size:2rem;margin-bottom:10px;opacity:0.5"></i>暂无规则</div></div></div></div><div id="setModal" class="modal"><div class="modal-box"><div class="tab-header" style="display:flex;gap:20px;margin-bottom:15px;cursor:pointer"><div class="tab-btn active" onclick="switchTab(0)" id="t0">安全</div><div class="tab-btn" onclick="switchTab(1)" id="t1">外观</div></div><div class="tab-content active" id="tab0"><label>用户名</label><input id="set_u" value="{{USER}}"><label>新密码</label><input id="set_p" type="password"><div style="margin-top:20px;display:flex;justify-content:flex-end;gap:10px"><button class="btn btn-gray" onclick="closeModal()">取消</button><button class="btn btn-primary" onclick="saveAccount()">更新</button></div></div><div class="tab-content" id="tab1" style="display:none"><label>PC背景 URL</label><input id="bg_pc" value="{{BG_PC}}"><label>移动端背景 URL</label><input id="bg_mob" value="{{BG_MOBILE}}"><div style="margin-top:20px;display:flex;justify-content:flex-end;gap:10px"><button class="btn btn-gray" onclick="closeModal()">取消</button><button class="btn btn-primary" onclick="saveBg()">应用</button></div></div></div></div><div id="editModal" class="modal"><div class="modal-box"><h3>编辑规则</h3><input type="hidden" id="edit_id"><label>备注</label><input id="edit_n"><label>监听</label><input id="edit_l"><label>目标</label><input id="edit_r"><div style="margin-top:20px;display:flex;justify-content:flex-end;gap:10px"><button class="btn btn-gray" onclick="closeModal()">取消</button><button class="btn btn-primary" onclick="saveEdit()">保存</button></div></div></div><script>let rules=[];const $=id=>document.getElementById(id);async function load(){const r=await fetch('/api/rules');if(r.status===401)location.href='/login';const d=await r.json();rules=d.rules;render();window.dispatchEvent(new Event('resize'))}function render(){const t=$('list');const ev=$('emptyView');const table=$('ruleTable');t.innerHTML='';if(rules.length===0){ev.style.display='block';table.style.display=window.innerWidth<768?'none':'table'}else{ev.style.display='none';table.style.display='table';rules.forEach(r=>{const row=document.createElement('tr');if(!r.enabled)row.className='row-paused';row.innerHTML=`<td data-label="状态"><span class="status-dot ${r.enabled?'bg-green':'bg-gray'}"></span>${r.enabled?'运行':'暂停'}</td><td data-label="备注"><strong>${r.name||'-'}</strong></td><td data-label="监听">${r.listen}</td><td data-label="目标">${r.remote}</td><td data-label="操作" style="text-align:right;display:flex;justify-content:flex-end;gap:5px"><button class="btn btn-sm ${r.enabled?'btn-gray':'btn-primary'}" onclick="tog('${r.id}')"><i class="fas ${r.enabled?'fa-pause':'fa-play'}"></i></button><button class="btn btn-sm btn-primary" onclick="openEdit('${r.id}')"><i class="fas fa-edit"></i></button><button class="btn btn-sm btn-danger" onclick="del('${r.id}')"><i class="fas fa-trash"></i></button></td>`;t.appendChild(row)})}}async function add(){let [n,l,r]=['n','l','r'].map(x=>$(x).value);if(!l||!r)return;if(!l.includes(':'))l='0.0.0.0:'+l;await fetch('/api/rules',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:n,listen:l,remote:r})});['n','l','r'].forEach(x=>$(x).value='');load()}async function tog(id){await fetch(`/api/rules/${id}/toggle`,{method:'POST'});load()}async function del(id){if(confirm('删除?'))await fetch(`/api/rules/${id}`,{method:'DELETE'});load()}function openEdit(id){const r=rules.find(x=>x.id===id);$('edit_id').value=id;$('edit_n').value=r.name;$('edit_l').value=r.listen;$('edit_r').value=r.remote;$('editModal').style.display='flex'}async function saveEdit(){const body=JSON.stringify({name:$('edit_n').value,listen:$('edit_l').value,remote:$('edit_r').value});await fetch(`/api/rules/${$('edit_id').value}`,{method:'PUT',headers:{'Content-Type':'application/json'},body});$('editModal').style.display='none';load()}function openSettings(){$('setModal').style.display='flex';switchTab(0)}function closeModal(){document.querySelectorAll('.modal').forEach(x=>x.style.display='none')}function switchTab(idx){$('tab0').style.display=idx===0?'block':'none';$('tab1').style.display=idx===1?'block':'none';$('t0').classList.toggle('active',idx===0);$('t1').classList.toggle('active',idx===1)}async function saveAccount(){await fetch('/api/admin/account',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:$('set_u').value,password:$('set_p').value})});alert('已更新，请重新登录');location.reload()}async function saveBg(){await fetch('/api/admin/bg',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({bg_pc:$('bg_pc').value,bg_mobile:$('bg_mob').value})});location.reload()}window.addEventListener('load',()=>{load();setTimeout(()=>window.dispatchEvent(new Event('resize')),300)});</script></body></html>
"#;
EOF

# 4. 编译安装 
echo -e -n "${CYAN}>>> 编译面板程序...${RESET}"
