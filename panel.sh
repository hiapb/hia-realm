...
clear
echo -e "${GREEN}==========================================${RESET}"
echo -e "${GREEN}Realm 面板 (安全优化+完美UI修复版)   ${RESET}"
echo -e "${GREEN}==========================================${RESET}"
...
async fn login_action(cookies: Cookies, State(state): State<Arc<AppState>>, Form(form): Form<LoginParams>) -> Response {
    ...
    if form.username == data.admin.username && form.password == data.admin.pass_hash {
        ...
        cookie.set_same_site(tower_cookies::cookie::SameSite::Strict); // 安全增强
        cookies.add(cookie);
...
tbody tr:hover td{background:rgba(255,255,255,0.7);transform:translateY(-1px);box-shadow:0 4px 10px rgba(0,0,0,0.02)}
